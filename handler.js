...
/*------------------------------------------------*/
		...
		if (!user.wait) user.wait = 0    
		    ...
		    if (akinator) {
				if (!('session' in akinator)) akinator.sesi = false
				...
				if (!('steps' in akinator)) akinator.step = null
				if (!('sunny' in akinator)) akinator.soal = null
	            } else
		        global.db.data.users[m.sender].akinator = {
				sesi: false,
				server: null,
  ...
  user.commandCount = 1
	        ...
	        let hl = _prefix 
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `*[❗𝐈𝐍𝐅𝐎 ❗] YOUR DIAMONDS ARE SOLD OUT, YOU CAN BUY MORE USING THE COMMAND ${usedPrefix}buy <cantidad>*`, m)
...
this.sendMessage(id, { text: `*[❗] @${user.split('@')[0]} ARABIC OR RARE NUMBERS ARE NOT ALLOWED IN THIS GROUP, SO YOU WILL BE TAKEN FROM THE GROUP*`, mentions: [user] }, { quoted: fkontak2 });          
return    
}    
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 👑 This command can only be used by the *Creator ░┋➠ 𝕎𝕒𝕛𝕒𝕔𝕜𝕠𝕪𝕒™┋*',
        owner: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] 👑 This command can only be used by the *Creator ░┋➠ 𝕎𝕒𝕛𝕒𝕔𝕜𝕠𝕪𝕒™┋*',
